 ...
 * the caller applies over the CURRENT params — so `?new=1` and anything else on
 * the page survives.
 *
 * `view` is ALWAYS written, even for the default. That costs a `?view=list` in
 * the address bar once somebody has touched the switcher, and buys the one
 * thing that matters: the URL is never silent about which view is showing. The
 * page's own server render reads the same params, and a key that is sometimes
 * absent-meaning-default and sometimes absent-meaning-stale is exactly how a
 * component ends up rendering last session's view.
 *
